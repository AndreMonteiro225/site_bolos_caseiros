...
export default function AdminDashboard() {
  ...
  const deletarPedido = async (id) => {
    ...
    const confirmar = window.confirm("Tem certeza que deseja excluir este pedido permanentemente?");
    if (!confirmar) return;
                ...
                <div className={styles.itensList}>
                  ...
                  <ul>
                    {pedido.itens && pedido.itens.map(item => (
                      <li key={item.id}>
                        <strong>{item.quantidade}x</strong> {item.nome_bolo} ({item.tipo === 'inteiro' ? 'Inteiro' : 'Fatia'})
